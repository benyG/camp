            ...
            ->schema([
                ...
                Forms\Components\Section::make(__('form.gs'))->columns(3)
                    ->description(function (Get $get) {
                        ...
                        : ' '.__('form.mqu').' '.match (auth()->user()->ex) {
                            1 => $ix->maxs,0 => '(inf.)',
                            2 => $ix->maxs, 3 => $ix->maxu, 4 => $ix->maxp, 5 => $ix->maxv,9 => $ix->maxeg,default => $ix->maxs
                        };
                    })
                    ->schema([
                                ...
                                if ($get('typee') == '1') {
                                    ...
                                    if ($cert->count() > 0) {
                                        ...
                                        $te = match (auth()->user()->ex) {
                                            0 => 40000000,1 => $ix->maxts,2 => $ix->maxts,3 => $ix->maxtu,4 => $ix->maxtp ,5 => $ix->maxtv,9 => $ix->maxtg,default => $ix->maxts
                                        };
                                        $set('examods', $cert->mods);
                                        $set('timer', $cert->timer >= $te ? $te : $cert->timer);
                                        $set('quest', $cert->quest >= $qe ? $qe : $cert->quest);
                                    } else {
                                        Notification::make()->warning()->title(__('forme.e13'))->send();
                                    }
                                }
                            })
                            ->required()->live(),
                            ...
                            ->live()->afterStateUpdated(function (?string $state, ?string $old, Get $get, Set $set) {
                                ...
                                if ($state == '1') {
                                    ...
                                    if ($cert->count() > 0) {
                                        ...
                                        Notification::make()->warning()->title(__('form.e13'))->send();
                                    }
                                }
                            }),
                                    ...
                                    ->action(function (Set $set, $state, Get $get) {
                                        ...
                                        if ($si > 0) {
                                            ...
                                            if (count($arrk) > 0) {
                                                ...
                                                Notification::make()->danger()->title(__('form.e15'))->send();
                                            }
                                        } else {
                                            Notification::make()->danger()->title(__('form.e15'))->send();
                                        }
