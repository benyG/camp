        ...
        return $table
            ...
            ->actions([
                    ...
                    ->using(function (\Illuminate\Database\Eloquent\Model $record, array $data): \Illuminate\Database\Eloquent\Model {
                        ...
                        if ($record->wasChanged()) {
                            ...
                            if ($record->type != $reco->type) {
                                $txt .= "Type of users was changed from '".str_replace(['1', '2', '3', '4', '5'], ['Admin', 'Starter', 'User', 'Pro', 'VIP'], implode(',', $reco->type))."' to '".
                                    str_replace(['1', '2', '3', '4', '5'], ['Admin', 'Starter', 'User', 'Pro', 'VIP'], implode(',', $record->type))."' <br>";
                            }
                            if ($record->wasChanged('text')) {
                                $txt .= 'The content was changed. <br>';
                            }
                            if (strlen($txt) > 0) {
                                \App\Models\Journ::add(auth()->user(), 'Announcements', 3, 'Announcement ID '.$record->id.'<br>'.$txt);
                            }
                        }
                ...
                Tables\Actions\DeleteAction::make()->iconButton()->after(function ($record) {
                    ...
                    Url: $record->url <br>
                    Users: ".str_replace(['1', '2', '3', '4', '5'], ['Admin', 'Starter', 'User', 'Pro', 'VIP'], implode(',', $record->type)).' <br>
