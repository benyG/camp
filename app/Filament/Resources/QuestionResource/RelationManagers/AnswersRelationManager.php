                        ...
                        \Filament\Forms\Components\Actions\Action::make('kkj')->label(__('form.att'))
                            ...
                            ->action(function (Get $get, $state) {
                                if (empty($get('text'))) {
                                    ...
                                    if (! Answer::where('text', $get('text'))->exists()) {
                                        Notification::make('e7')->danger()->title(__('form.e22'))->send();
                                    } else {
                                        $ans = Answer::where('text', $get('text'))->first();
                                        $quest = $this->getOwnerRecord();
                                        if ($quest->answers()->get()->contains($ans)) {
                                            Notification::make('er')->danger()->title(__('form.e23'))->send();
                                        } elseif ($quest->answers()->count() >= $quest->maxr) {
            ...
            ->actions([
                Tables\Actions\EditAction::make()->using(function (\Illuminate\Database\Eloquent\Model $record, array $data): \Illuminate\Database\Eloquent\Model {
                    ...
                    if ($record->wasChanged()) {
                        ...
                        if (strlen($txt) > 0) {
                            \App\Models\Journ::add(auth()->user(), 'Answers', 3, $txt);
                        }
                    }
